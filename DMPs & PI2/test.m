...
rbf_dmp.bf_type = "rbf";
%rbf_dmp.no_exp = 1;
mullifier_dmp = dmp(alpha,damping_coefficient,mass,n_rbf,sigma,n_time_steps);
%mullifier_dmp.no_exp = 1;
rbf_dmp = rbf_dmp.train("trajectory.txt");

mullifier_dmp = mullifier_dmp.train("trajectory.txt");
mullifier_dmp.plot_rollout(0)

writematrix([rbf_dmp.mean_x,rbf_dmp.mean_y],"results/orig_rbf_means.txt")
writematrix([mullifier_dmp.mean_x,mullifier_dmp.mean_y],"results/orig_mullifier_means.txt")
