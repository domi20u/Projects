%% Reproduce the rotodilatation results from DMP++
...
sigma = 3;
time_steps = 1000;
dmp_scaled = dmp(alpha,D,mass,n_bf,sigma,time_steps);
...
dmp_not_scaled.plot_trajectory(0)
dmp_scaled_rbf.plot_trajectory(0)
plot(new_goal(1),new_goal(2),'k+')
legend("original","scaled mollifier","not scaled","scaled rbf","new goal")
...
dmp_scaled_rbf.plot_trajectory(0)
plot(new_goal(1),new_goal(2),'k+')
legend("original","scaled & new x-end & x-init","not scaled & new x-end","scaled & new x-end","new goal")
